...
import { User } from '../../../../entities/gestion_usuarios/user';
import UserRepository from './../../../../persistence/repositories/gestion_usuarios/user.repository';
...
class UserController {
    ...
    public getUser(request: Request, response: Response) {
        ...
        UserRepository.findUser(request.body.id).then(user => {
            response.status(200).json({status: true, data: user});
        }, error => {
            response.status(404).json({status: false});
        });
